...
class AhpkeringController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        if ($data_sensor3->isEmpty()) {
            $errorMessage = "Tidak ada data sensor yang tersedia.";
            return view('ahp.ahpkering', compact('errorMessage'));
        }

        // Hitung rata-rata nilai sensor
        $ph_tanah_avg = $this->calculateAverage($data_sensor3->pluck('ph_tanah')->toArray());
        $temperature_avg = $this->calculateAverage($data_sensor3->pluck('temperature')->toArray());
        $nilai_stanah_avg = $this->calculateAverage($data_sensor3->pluck('nilai_stanah')->toArray());
        $nilai_lembab_avg = $this->calculateAverage($data_sensor3->pluck('nilai_lembab')->toArray());
        ...
        $rankingKering = $this->calculateAHP($ph_tanah_avg, $temperature_avg, $nilai_stanah_avg, $nilai_lembab_avg, $weights, $subcriteria, $alternatives);

        return view('ahp.ahpkering', compact('rankingKering'));
    }
